...
impl<T: TextStorage> RawLabel<T> {
    ...
    pub fn with_and_color(color:Color) -> Self {
        let mut text_layout = TextLayout::new();
        text_layout.set_text_color(color.clone());
        Self {
            layout: text_layout,
            line_break_mode: LineBreaking::Overflow,
            disabled: false,
            default_text_color: color.into()
        }
    }
...
impl<T: Data> Label<T> {
    ...
    pub fn with_and_color(text: impl Into<LabelText<T>>,text_color:Color) -> Self {
        let text = text.into();
        let current_text = text.display_text();
        Self {
            text,
            current_text,
            label: RawLabel::with_and_color(text_color),
            text_should_be_updated: true
        }
    }
